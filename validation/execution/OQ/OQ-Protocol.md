# OQ Protocol — Platform Core 1.0-RC2

| Field | Value |
| --- | --- |
| Document | VAL-OQ-PC-RC2 |
| Baseline | PDF-PC-VAL-BL-1.0 |
| Candidate | 1.0-RC2 (`platform-core-v1.0-rc2` → e2b2297a1506603ba05e6fb1dcf973ac046ad009) |
| Validation status | NOT_VALIDATED |
| Execution | **EXECUTED 2026-08-22** (partial; overall BLOCKED) |

Operational Qualification challenges specified Platform Core functions on the installed RC2 candidate. Developer unit tests are not OQ evidence.

Formal OQ executed 2026-08-22 against immutable tag `platform-core-v1.0-rc2`. IQ prerequisite: PASS WITH OPEN OBSERVATIONS; IQ-FIND-001 CLOSED. Product validation status remains **NOT_VALIDATED**. UAT remains **NOT_EXECUTED**.

Overall result: **BLOCKED** (OQ-FIND-001, OQ-FIND-002). Evidence: `validation/execution/evidence/OQ/RC2/`.
...
| Actual Result | BLOCKED — GitHub OAuth env present but no interactive Catalog User session (auth sessions=0). OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | Unauthenticated POST scaffolder/tasks, GET entitlements admin, POST certification, GET catalog/entities all returned HTTP 401 AuthenticationError Missing credentials. Evidence OQ-AUTH-002-*. |
| Status | PASS |
...
| Actual Result | BLOCKED — no Viewer session. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no Developer session. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no Owner session. OQ-FIND-001. |
| Status | BLOCKED |
...
| Evidence Required | HTTP/UI results |
| Actual Result | BLOCKED — no Admin session. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no Viewer session for mutation denial. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no entitled Developer session. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no not-entitled Developer session. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — ENTITLEMENT_PROVIDER local/default; AWS/marketplace-test profile not active. OQ-FIND-002. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — interactive Create requires authenticated entitled Developer + GitHub App publish path. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — denied Create requires Viewer / not-entitled Developer sessions. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — depends on OQ-SCF-001/002; audit file contains IQ probe lines only, not Create GRANT/DENY. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no Create GRANT record from OQ Create. OQ-FIND-001. |
| Status | BLOCKED |
...
| Evidence Required | Redacted record |
| Actual Result | BLOCKED — no Create DENY record from OQ Create. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no OQ GRANT/DENY records to re-retrieve after restart. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — controlled torn-line mutation of candidate audit not performed without authenticated admin retrieval path / change control. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — ci-status requires authenticated data-product view; no session; live PASS Actions entity not exercised. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — no authenticated CI FAIL retrieval. OQ-FIND-001. |
| Status | BLOCKED |
...
| Evidence Required | Status payload |
| Actual Result | BLOCKED — no authenticated CI UNKNOWN retrieval. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — UI DEGRADED/UNVERIFIED label not observed on live authenticated UI (string present in compiled bundle only). OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — live App publish/Actions read not executed without authenticated Create/CI path. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | Scanned 9 committed app-config/docker-compose files at RC2 tag: 0 raw secret hits; 30 ENV placeholders for secret-like keys. Evidence OQ-SEC-001-*. |
| Status | PASS |
...
| Actual Result | Scanned 393 compiled JS files in image dist/static: 0 private keys; 0 token-shaped ghp_/AKIA; no literal clientSecret values. Evidence OQ-SEC-002-*. |
| Status | PASS |
...
| Actual Result | Cross-check SEC-001/002: clientId permitted; clientSecret/private keys/tokens not frontend-embedded as secrets. |
| Status | PASS |
...
| Actual Result | BLOCKED — AWS/marketplace-test profile not active on validation stack (register NOT_CONFIGURED). OQ-FIND-002. Fail-closed flags observed on register but preconditions unmet for PASS. |
| Status | BLOCKED |
...
| Actual Result | Hosted catalog DB has 65 production entities including official Golden Path templates and platform components. HTTP catalog 401 without auth. Evidence OQ-CAT-001-*. |
| Status | PASS |
...
| Actual Result | Sample entity/location counts = 0 in hosted catalog DB. Evidence OQ-CAT-002-*. |
| Status | PASS |
...
| Actual Result | BLOCKED — authenticated walk of Composer/Release Catalog/Quality Gate/Marketplace/Data Product not possible without session. Compiled bundle shows 0 forbidden GMP/GxP/Part11 COMPLIANT claims and contains NOT_VALIDATED / not GxP; insufficient alone. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — authorize-create requires auth; LEGAL_DISTRIBUTION_STATUS=BLOCKED but customer/internal matrix not executed. Config not altered. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | Inventory confirmation: no Customer Package Export API/UI in product code (docs/policy mentions only). |
| Status | NOT_APPLICABLE_CURRENT_RELEASE |
...
| Actual Result | Confirmed: register is non-activating (`accessGranted=false`, `tenantCreated=false`, no Set-Cookie, status NOT_CONFIGURED). No separate activation operation. |
| Status | NOT_APPLICABLE_CURRENT_RELEASE |
...
| Actual Result | Inventory confirmation: no Marketplace commercial publish/release operation in product code. |
| Status | NOT_APPLICABLE_CURRENT_RELEASE |
...
| Actual Result | Confirmed tenant provisioning absent (`tenantCreated=false` on register contract and live response). |
| Status | NOT_APPLICABLE_CURRENT_RELEASE |
...
These tests were added so every active URS has a planned formal verification item. RC2 OQ statuses are recorded below.
...
| Actual Result | BLOCKED — no unmapped/no-role signed-in user session. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — Marketplace SPA shell loads unauthenticated but labeled cards require authenticated UI evidence/screenshots. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — /developer returns SPA shell 200; TechDocs metadata 401 without session; hosted TechDocs content not opened authenticated. OQ-FIND-001. |
| Status | BLOCKED |
...
| Actual Result | BLOCKED — unauthenticated search returns 401 (negative check only); authorized Catalog/TechDocs search not executed. OQ-FIND-001. |
| Status | BLOCKED |
