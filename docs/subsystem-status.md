...
| URS Composer | `plugins/urs-composer`, `plugins/urs-composer-backend` | **TESTED** | **DRAFT** — not a released customer module | **FUTURE** (no SKU) | **NOT VALIDATED** | Requirements, baselines, approval chain, e-sign *mechanism*. E-sign UI + change-request UI productized as **technical controls** only. Intended use: [validation/urs-composer/intended-use.md](validation/urs-composer/intended-use.md). Outside MVP 1.0 value proposition. |
